...
| 低コスト | **月額 $15 以下を目標** (2026-05-23 実測ベースで見直し。家族のみ運用の実コストは月 $5 前後で、Sonnet 4.6 + Haiku 4.5 が大半を占める。Ragas 月次評価と Phase 2 拡張ヘッドルームを見て $15。詳細は `docs/knowledge.md` 2026-05-23 参照)。S3 Vectors（完全従量課金）採用。OpenSearch Serverless（月 $175〜$350 固定費）は採用しない。 |
