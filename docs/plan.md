...
ペット鶏 150羽との暮らし（自作鶏小屋 20m × 6m + 害獣対策 + 卵 30個/日 の活用）を支援するRAGエージェントを家族2名で利用するプライベートシステムとして構築する。鶏の生命と卵の食品安全の両面で命を扱う性質上、精度担保を最優先とする。

スコープA+B+C は完走済み。Phase 1.5 (Cognito認証 + マルチスレッド + ナレッジ投稿フォーム + Amplify Hosting デプロイ) を経て、奥さんへの公開URL共有が次の到達点。
...
- 配偶者の利用開始想定時期（MVPの完成目標日）— 鶏小屋完成 (2026-06頃) と並行で Phase 1.5 を進める想定
- 害獣対策: 物理防御フェーズ (現状) → 忌避フェーズ (鶏小屋完成後) で必要なナレッジが変化する。Phase 1.5 のナレッジ投稿フォームでフェーズ別に蓄積
- 所在都道府県（茨城・千葉等）— 害獣対策の自治体マニュアル取込時に必要
- 鶏の品種 — ナレッジテンプレートの breed 項目で絞り込み可能にしたい
