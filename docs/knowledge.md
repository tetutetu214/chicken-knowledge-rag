...
- **2026-05-26: Query Expansion (語彙拡張) の本質** — ユーザの日常語 (例「首の骨」) と KB の専門語 (例「頚椎」) の **語彙ギャップ** を、LLM で同義語/別表現を生成して複数クエリで Retrieve することで埋める手法。語彙が違うだけで意味は同じ質問が「topScore 0.622」のように閾値以下に沈むことを救済する。スペルミス修正でも一般知識 fallback でも API コスト削減でもなく、純粋に「retrieval の検索範囲を広げる」のが目的。RAG コミュニティの標準テクニックで、業界では Query Expansion / Multi-Query / Query Rewriting などと呼ばれる。LangChain の `MultiQueryRetriever` 等に標準実装あり。
- **2026-05-26: HyDE と Query Expansion は別の問題を解く別ツール (上位互換関係ではない)** — HyDE (Hypothetical Document Embeddings) は「LLM に仮想の回答文を生成させ、その仮想文の埋め込みベクトルで検索する」手法で、**質問とドキュメントの記述スタイル乖離** (例: 質問は短い疑問形、KB は長い説明調) を解く。Query Expansion は **語彙カバレッジ** を解く。Cocco RAG の「鶏の首の骨 (topScore 0.622) / 頚椎 (KB に答えあり)」 のような語彙ギャップ問題には **Query Expansion が直撃** する。HyDE は別の問題を解くので、ここで採用しても遠回り。両者は意味的に独立した手法で「HyDE が新しい上位互換」ではない。Adaptive RAG (2026 年現在の業界ベストプラクティス) は質問の特性で動的に手法を切り替える。
- **2026-05-26: Conditional Retry (取りこぼし帯だけ拡張) のトレードオフ** — 「全質問で必ず Query Expansion」(A/B 案) は実装シンプルだが、KB ヒット明確な質問にも Bedrock 呼出が必ず増えてコスト+レイテンシが乗る。**Conditional Retry (C 案)** は `topScore` 3 帯振り分け (0.7 以上 = ヒット明確 / 0.62〜0.69 = 取りこぼし疑い / 0.62 未満 = KB に無い) で、**中間帯だけ Nova Pro 呼出+追加 Retrieve** する。代償は (1) 実装の場合分けが増えること、(2) 上下 2 閾値を運用調整する必要があること。家族 100 質問/月のうち中間帯は推定 20 件で、追加コストは月数十円規模。観測駆動で `/insights` から救済率を月次で見て閾値を調整できる構造になる。
- **2026-05-26: AWS Bedrock Knowledge Bases 標準 Query Decomposition は `RetrieveAndGenerate` 専用** — Bedrock KB には組み込みで `orchestrationConfiguration.queryTransformationConfiguration.type = 'QUERY_DECOMPOSITION'` というオプションがあり、複雑な複合質問を自動でサブクエリ分解して並列 Retrieve してくれる。**ただし `RetrieveAndGenerate` API の中でしか使えない**。Cocco RAG は `Retrieve` + `Converse` 自前構成 (knowledge.md 2026-05-04 参照、systemPrompt のリスク階層化・コケ語尾・KB ヒット閾値振り分け・累積要約を Lambda 内で制御するため) なので、組み込み機能は使えず自前実装する必要がある。ただし「自前実装 = 公式と本質的に同じロジックを Lambda 内で書く」だけで、Bedrock 課金は同じ (LLM 呼出 + 複数 Retrieve)。「便利機能のパッケージング」が違うだけで「使えない技術」ではない。
- **2026-05-26: stacked PR は base ブランチ削除で自動 close される + reopen 不可** — GitHub の挙動: 子 PR (PR #63) の base が親 PR (PR #62) のブランチを指している状態で、親 PR を squash merge して base ブランチを削除すると、子 PR は **GitHub が自動で CLOSED 状態にする**。さらに「base ブランチが存在しない PR は reopen 不可」(GraphQL エラー `Could not open the pull request`)。base 変更 API も「Cannot change the base branch of a closed pull request」で拒否される。**回避策は子 PR を新規 PR として再作成すること** (HEAD ブランチは生きているので、main を base にして新規 `gh pr create` 可能)。stacked PR を組む場合、親をマージする前に子の base を main に手動で切り替えておく (`gh api -X PATCH .../pulls/{N} -f base=main`) のが安全 (本件で起きた事故、PR #63 → #64 再作成、2026-05-26)。
