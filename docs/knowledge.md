...
- **2026-06-02: UI 要素を動かす/消す前に「テストの成功判定が何に依存しているか」と「なぜ足されたか」を確認する** — サインアウトボタンは `auth.setup.ts` のログイン成功判定 (可視チェック) に使われているため、メニューに隠して不可視にすると E2E が壊れる。要素を移動・非表示にする前に locator・判定での利用有無を確認する。また「邪魔なボタンを消す」前に、それが何の問題への対処で足されたか (今回は iPhone 下部ツールバー被り) を確認しないと元の問題が再発する。下端被りは要素を画面上部へ移すと物理的に解消できる。
...
### 2026-06-02: サインアウトボタンをサイドバー上部1か所に集約 (右上フロート廃止)

- **決定**: スマホ用に画面右上へ常時表示していたサインアウトのフロートボタン (`fixed top-3 right-3`) を廃止し、サインアウトをサイドバー内1か所に集約。アカウント欄 (ユーザー名 + サインアウト) をヘッダー (🐓 Cocco RAG) の直下に移設した (`web/app/page.tsx`)。
- **背景**: 旧構成ではサインアウトが (a) サイドバー下部固定 と (b) 画面右上フロート の2か所に散らばっていた。フロートは家族から「常に1つだけ浮いていて邪魔・誤タップしやすい」とフィードバック。
- **元のフロートを足した理由と、その解消**: フロートはもともと「サイドバー下端のサインアウトが iPhone Safari/Chrome の下部ツールバーに被って押せない」問題への対処だった。今回アカウント欄を**ヘッダー直下 (サイドバー上部)** に移したことで、サイドバーを開いた瞬間に画面上部に出て下部ツールバー被りが原理的に発生しなくなり、フロートと下端の被り対策余白 (`pb-[calc(env(safe-area-inset-bottom)+4rem)]`) の両方を撤去できた。
- **設計上の制約**: `tests/auth.setup.ts` はログイン成功を「サインアウトボタンが可視」で判定しているため、サインアウトをメニューの奥に隠す案は不可。常時可視のままサイドバー内に置く形にした (E2E もそのまま通過)。
- **検証**: `tsc --noEmit` EXIT 0 / Vitest 52 件 pass / Playwright E2E 11 件 pass (auth.setup・insights・mobile・passkey 含む)。差分は page.tsx のみ (+20/−36)。
