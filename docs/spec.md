# 鶏 知識統合RAGエージェント 要件定義書 v2.0

> Chicken Knowledge RAG System — Requirements Specification
>
> | 項目 | 値 |
> |---|---|
> | 作成日 | 2026-05-02 (v1.0) / 2026-05-03 (v2.0 ペット飼育前提に改訂) |
> | バージョン | v2.0 |
> | ステータス | スコープA+B+C完走、Phase 1.5 着手前 / Claude Code 実装用 |
> | 利用者 | 家族（うち1名は鶏 飼育 3年経験者） |
> | 対象規模 | 100羽以上の鶏（家畜伝染病予防法上の届出対象規模） + 自作鶏小屋（建築中） + 副産物としての卵活用 |

---

## 0. システム概要

本システムは「100羽以上の鶏との暮らし」を支援する RAG ベースの AI エージェントである。
家族のプライベート用途で運用し、命を扱う性質と日常運用の両面から精度担保を最優先設計方針とする。

### 0-1. 想定する利用シーン

- **飼養衛生管理**: 家畜伝染病予防法上、100羽以上は届出対象。ペット用途でも防疫義務あり
- **鶏小屋の自作・改修**: AW指針平飼い推奨水準を満たす広さで建築中。
  害獣対策の物理防御フェーズが進行中（金網設置・あぜ道シート埋設による地下侵入防止）
- **害獣対策**: カラス/タヌキ/ハクビシン/イタチ。
  現状フェーズ＝物理防御、小屋完成後フェーズ＝忌避剤・音響忌避・継続観察
- **動物福祉(AW)・行動観察**: 日光浴・運動・社交・個体識別・性格把握などの飼育
- **卵の活用**: 日々得られる卵の処理。保存・賞味期限・大量消費レシピ・食中毒予防

### 0-2. ナレッジ統合の核心

公的マニュアル（飼養衛生管理基準・HPAI防疫指針・AW指針 等）を権威基盤とし、
家族の現場知識（鶏小屋建築・害獣対策・観察記録・卵料理・ペット行動）を Markdown で
蓄積する三層構成のナレッジベースに、マルチスレッドチャットUI を組み合わせる。

養鶏業ではなくペット飼育のため、産業用 HACCP / 生産性指標は副次的に扱う。
代わりに、個別の鶏との関係性（個体識別・性格・健康履歴）や、日常生活への組み込み
（卵料理レシピ・鶏小屋設備の DIY 記録・ペット行動の観察日記）を主要ドメインとする。
...
鶏の生命と、家族・第三者が消費する卵の食品安全の両面で、命を扱う性質を持つ。

- ハルシネーション抑制を全設計判断の最上位に置く
- 回答には必ず出典（ドキュメント名・ページ番号・引用箇所）を明示する
- 「知らない」「確認が必要」と答えることを推奨する応答スタイルを採用する
- RAGAS 等による定期的な評価パイプラインを設計に組み込む
- 以下のカテゴリでは必ず専門家確認を促すアラートを回答に付加する:
  - **疾病・薬剤・緊急対応** → 獣医師・家畜保健衛生所への連絡を促す
  - **食品安全（卵の取扱い・保存・賞味期限超過）** → 食中毒リスクを明記、保健所相談を促す
  - **害獣被害** → 鳥獣保護管理法上の捕獲制限、自治体への確認を促す

> ⚠️ 鳥インフルエンザ・コクシジウム等の感染症、ニューカッスル病ワクチン等の薬剤投与に関する回答は、農水省飼養衛生管理基準の該当箇所を必ず引用したうえで、専門家への確認を促す文言を付加すること。

> ⚠️ サルモネラ等の卵関連食中毒・賞味期限超過卵の取扱い・大量消費レシピ等の食品安全に関する回答は、厚労省食品衛生基準・自治体保健所の指針を引用し、家族以外への譲渡時は譲渡先への注意喚起を促すこと。
...
| Cognito User Pool | 認証 | 2名のユーザーのみ。Amplify UI `<Authenticator>` で実装。2026-05-17 から Passkey (WebAuthn) 併用導入 (詳細は §4-2-1) |
...
|---|---|---|---|
| 最高 | 公的権威ソース | 飼養衛生管理基準(令和7年9月)、HPAI防疫指針(令和7年10月)、AW指針(採卵鶏編)、鶏卵生産衛生管理ハンドブック、HACCP衛生管理マニュアル、厚労省・自治体の卵衛生通知、鳥獣被害対策マニュアル | `official_regulation` |
| 高 | 精選学術論文 | J-STAGE（Journal of Poultry Science）、Poultry Science誌OA論文。鶏の福祉・行動学・疾病・卵衛生・害獣防除に関するもの20〜30本に厳選 | `research_paper` |
| 最高 | 現場ナレッジ | 家族が書くMarkdown形式のノウハウ・観察記録・SOP・ワクチン記録・害獣対策記録・鶏小屋DIY記録・個体観察日記・卵料理レシピ等 | `field_knowledge` |

> ⚠️ 「意味的に近いが誤情報を含む文書（ノイズ文書）」はRAG精度の最大の脅威（arXiv:2401.14887）。論文は必ず精選すること。本システムの対象は **100羽以上の家庭飼育規模・自作鶏小屋・日本の気候条件** であり、それから乖離した条件（大規模ケージ飼い・熱帯気候・他畜種等）の論文は取り込まない。

### 3-2. 取得すべき一次ソース一覧

#### 公的マニュアル — 既取込 (スコープA で配備済み)
...
#### 公的マニュアル — 追加取込候補 (Phase 1.5 / その後)

| カテゴリ | ドキュメント名 | URL候補 |
|---|---|---|
| 卵衛生 | 厚労省「鶏卵の衛生対策について」 | https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/kenkou_iryou/shokuhin/ |
| 卵衛生 | 食品衛生法「食品、添加物等の規格基準」鶏卵関連 | https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/kenkou_iryou/shokuhin/jigyousya/shokuhin_kikaku/ |
| 害獣対策 | 農水省「鳥獣被害対策コーナー」関連資料 | https://www.maff.go.jp/j/seisan/tyozyu/higai/ |
| 害獣対策 | 環境省「鳥獣保護管理法」運用指針 | https://www.env.go.jp/nature/choju/index.html |
| 害獣対策 | 各自治体（茨城・千葉・埼玉等）のハクビシン/タヌキ対策マニュアル | （所在都道府県を確認してから） |
...
> 論文の選定基準（v2.0 改訂）: ① 採卵鶏（肉用鶏は除外）、② 日本国内の飼育条件または汎用飼育技術、③ ペット飼育に親和的な領域(疾病・行動学・福祉・卵衛生・害獣防除等)。④ 大規模ケージ飼い前提の生産性論文は対象外。20〜30本に厳選。
...
category: [育成/飼料/疾病/衛生/害獣対策/鶏小屋建築/鶏小屋設備/産卵/卵料理/個体観察/行動・福祉]
breed: [ボリスブラウン/もみじ/他]  # 任意
age_weeks: xx  # 週齢、特定個体に紐づく場合のみ（数値型）
individual_id: A-001  # 個体識別、ペット観察記録の場合（任意）
location: 第1鶏舎  # 任意
season: [春/夏/秋/冬]  # 任意、害獣対策などで季節依存性が強い場合
tags: [ワクチン, ニューカッスル病, ハクビシン, 金網, 卵焼き, 大量消費]  # コントロールドボキャブラリ使用
---
# 概要（必須・冒頭に1段落の要約を書く）
〇〇について。句点で文を終える。
...
#### カテゴリ別の主要トピック例 (Phase 1.5 のナレッジ投稿フォームでも参照する)

| カテゴリ | 想定トピック例 |
|---|---|
| 育成 | 雛の温度管理、初給餌、ワクチンスケジュール |
| 飼料 | 配合変更、季節別補助飼料、家庭の野菜くず活用 |
| 疾病 | 死亡事例、症状観察、獣医相談記録、ワクチン履歴 |
| 衛生 | 鶏舎清掃ローテーション、消毒記録、踏込槽運用 |
| 害獣対策 | カラス/タヌキ/ハクビシン/イタチの被害、侵入経路、物理防御の工夫、忌避剤の効果検証 |
| 鶏小屋建築 | 自作鶏舎の DIY ログ、寸法・材料・費用、地下対策、雨樋 |
| 鶏小屋設備 | 巣箱・止まり木・換気・採光・止まり板、改修記録 |
| 産卵 | 産卵率の季節変動、巣箱の好み、放卵防止 |
| 卵料理 | 卵の大量消費レシピ、保存方法、賞味期限管理、自家製マヨネーズ等 |
| 個体観察 | 個体識別 (個体名/番号)、性格・気質、健康履歴、相性 |
| 行動・福祉 | 日光浴のローテーション、運動・砂浴び、社会性観察、ペットとしての触れ合い |
...
### 4-2-1. パスキー (WebAuthn) 認証 (2026-05-17 追加)

家族のログイン体験を改善するため、Cognito ネイティブ WebAuthn を email/password と併用する。詳細な導入計画と判断記録は `plan.md` の「Passkey 認証導入計画」セクション参照。

| 項目 | 内容 |
|---|---|
| 認証方式 | パスキー (WebAuthn) と email/password の併用 (移行期) → 将来パスキーのみ |
| Cognito ティア | Essentials 以上 (WebAuthn の前提) |
| `relyingPartyId` | Amplify Hosting 自動ドメイン (`xxxxx.amplifyapp.com`)。カスタムドメイン未設定 |
| `userVerification` | `required` (生体認証 or PIN を必須) |
| サインアップ | 現状維持 (家族はメアド+パスワードでサインアップ可能、Issue #32 は将来対応) |
| 登録UI | ログイン後にサイドバー「🔑 パスキー管理」ボタン → モーダルで `associateWebAuthnCredential()` |
| 一覧/削除UI | 同モーダル内で `listWebAuthnCredentials()` / `deleteWebAuthnCredential()` |
| ログイン画面 | Phase 3 で自前ログイン画面に差し替え (2026-05-18 方針確定)。1 画面にメアド入力欄 + 「🔑 パスキーでサインイン」(メイン) + 「パスワードでサインイン」リンク (フォールバック展開)。`signIn({ username, options: { authFlowType: 'USER_AUTH', preferredChallenge: 'WEB_AUTHN' } })` で生体認証を直接トリガー。家族全員パスキー登録完了後にフォールバックを撤去してパスキーのみへ切替 |
| エラーハンドリング | `UserCancelledException` / ブラウザ非対応 / 既登録デバイス重複 等を UI で吸収 |
...
| ナレッジ品質管理 | ドキュメント追加時のチェックリスト: source_type明記 / 鶏に関連か / 日本の飼育条件か / 100羽以上の家庭飼育に親和的か / 重複チェック |

### 5-2. 緊急時の回答ポリシー (v2.0 拡張)

| カテゴリ | 引用必須ソース | 必須付加文言 |
|---|---|---|
| 鳥インフルエンザ・コクシジウム等の感染症 | 農水省 HPAI 防疫指針 | 「家畜保健衛生所への連絡を必ず行ってください」 |
| ワクチン・薬剤投与 | 製品添付文書、農水省 飼養衛生管理基準 | 「獣医師への確認を必ず行ってください」 |
| 死亡事例・緊急症状 | 同上 | 回答冒頭に「直ちに獣医師・家畜保健衛生所に連絡してください」 |
| 害獣の捕獲・処分 | 環境省 鳥獣保護管理法 | 「無許可捕獲は法律違反。自治体・鳥獣保護管理法担当への確認を行ってください」 |
| 卵の食中毒・賞味期限超過卵の取扱い | 厚労省 食品衛生基準 | 「サルモネラ等の食中毒リスク有り。家族以外への譲渡時は譲渡先への注意喚起を行ってください」 |
| 大量消費レシピ・自家製マヨネーズ等の生卵使用 | 同上 | 「賞味期限内の卵に限る。妊婦・乳幼児・高齢者は加熱済みのみが推奨」 |

> 上記ポリシーは Bedrock Guardrails の「Denied topics」または「Custom responses」として設定し、プロンプトレベルに依存しない安全網として実装する。
>
> Phase 1.5 で Guardrails を本格設定する際、上記6カテゴリの自動検知ルールを優先実装する。
