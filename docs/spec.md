...
> | 利用者 | 家族のみ（うち1名はペット鶏 飼育 3年経験者） |
> | 対象規模 | ペット用 100羽以上 + 自作鶏小屋((規模情報) = 適切な面積、2026-06頃完成予定) + 副産物としての卵活用(約多数個/日) |

---

## 0. システム概要

本システムは「ペット鶏 100羽以上との暮らし」を支援する RAG ベースの AI エージェントである。
家族のみのプライベート用途で運用し、命を扱う性質と日常運用の両面から精度担保を最優先設計方針とする。

### 0-1. 想定する利用シーン

- **飼養衛生管理**: 家畜伝染病予防法上、100羽以上は届出対象。ペット用途でも防疫義務あり
- **鶏小屋の自作・改修**: (規模情報) = 適切な面積 (1羽あたり 0.8m²、AW指針平飼い推奨水準を満たす)。
...
- **卵の活用**: 多数個/日 = 約 多数/年 の処理。保存・賞味期限・大量消費レシピ・食中毒予防
...
> ⚠️ 「意味的に近いが誤情報を含む文書（ノイズ文書）」はRAG精度の最大の脅威（arXiv:2401.14887）。論文は必ず精選すること。本システムの対象は **100羽以上規模・ペット飼育・自作鶏小屋・日本の気候条件** であり、それから乖離した条件（大規模ケージ飼い・熱帯気候・他畜種等）の論文は取り込まない。
...
| 鶏小屋建築 | (規模情報) 鶏舎の DIY ログ、寸法・材料・費用、地下対策、雨樋 |
| 鶏小屋設備 | 巣箱・止まり木・換気・採光・止まり板、改修記録 |
| 産卵 | 産卵率の季節変動、巣箱の好み、放卵防止 |
| 卵料理 | 多数個/日 の大量消費レシピ、保存方法、賞味期限管理、自家製マヨネーズ等 |
...
| ナレッジ品質管理 | ドキュメント追加時のチェックリスト: source_type明記 / 鶏に関連か / 日本の飼育条件か / 100羽以上規模・ペット用途に親和的か / 重複チェック |
