...
最終更新: 2026-05-19 (Phase 3 PR #56 main マージ → Amplify Hosting 自動ビルドで本番反映完了 → PC Chrome で動作確認済み (パスキー登録・パスキーサインイン両方 OK))。実装過程で 2 つ大きな技術判断補正: ①パスワード fallback は `preferredChallenge: 'PASSWORD_SRP'` を明示しないと Cognito が `CONTINUE_SIGN_IN_WITH_FIRST_FACTOR_SELECTION` を返し 1 発で DONE にならない、②`useAuthenticator` は `route` ではなく `authStatus` を購読しないと Authenticator UI 不在時に更新されない (Hub auth イベントに反応するのは authStatus 側)。Android スマホ (Chrome) では「パスキーを追加」時に QR コード (Hybrid Transport) しか出ず、「このデバイスを使用」選択肢が出ない問題を確認中。主因仮説は Android 画面ロック未設定 (WebAuthn `userVerification: required` を満たせない) で、設定確認中。スマホ向けは現状パスワード fallback でログイン可能 (Phase 3 の fallback 設計が機能)。残課題は: (a) Android スマホパスキー UX 改善、(b) PasskeyError 系の生エラー文言「Passkey authentication/registration ceremony has been canceled」を日本語にマッピング (UserCancelledException 判定をすり抜ける別 PR で対応予定)。aws-mcp 認証は てつてつ手動で `aws login` 済み、`aws sts get-caller-identity = arn:aws:iam::522436770652:user/tetutetu`。次回再開: スマホ画面ロック設定の確認結果次第で対応分岐。前回: 同日 未明 Phase 2 パスキーフロント PR #55 main マージ → Amplify Hosting ビルド Job #42 SUCCEED)
