...
- [~] **ペルソナ「コケ語尾」必須化** (家族から「最近コケが入っていない」指摘。PR #38 で「1〜2回だけ」と上限のみ指定にした結果、Nova Pro がゼロ回解釈する事象が常態化。「必ず1回、最大2回」と下限を明示し、「一度も入れずに終えてはいけない」明示禁止行を追加。fix/koke-required、2026-05-11)
