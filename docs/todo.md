...
- ⏳てつてつ: aws login 後に Scheduler 記述の書き分け修正（実リソース確認とセット、audit-20260718 所見3）
- ⏳てつてつ: docs/photo/ の未追跡画像2枚の中身確認（公開リポのためコミット可否判断。audit-20260718 所見4）
- 家族体感系の着手判断（👎+自由記述 / クエリ拡張の再適用 / Phase3 分類）
- 雑務: AGENTS.md chore コミット + マージ済みブランチ6本の照合・掃除（audit-20260718 所見5・6）

最新状態を保つこと。完了したらチェックを入れて、必要なら新規タスクを追加する。詳細は `spec.md` §9 を参照。

## 次回再開時のチェックリスト

最終更新: 2026-05-27 (ペルソナ「鶏らしい表現」のレパートリー化と直前パターン連続使用禁止を `feature/chicken-character-repertoire` で対応。家族から「コケが毎回機械的すぎる」「コケコッコー等も入れるべき」のフィードバック → 2026-05-17 で位置固定化した「最後の1文必須」は維持、レパートリーを「〜コケ。」「〜コッコ。」「コケコッコー！」の 3 種類に拡張 + 直前ターンと同じパターンの連続使用を禁止。副次タスクとして CLAUDE.md の「回答生成LLM」表記を Sonnet 4.5/Haiku 4.5 → `apac.amazon.nova-pro-v1:0` に修正 (2026-05-08 に Nova Pro 完全移行済みだった事実が CLAUDE.md に反映されていなかったため)。詳細は knowledge.md 2026-05-27 参照。sandbox 動作確認は本番反映前に必要。/ 2026-05-24 (Issue #33 + #30 両方完了。**Issue #30** はスコープ縮小版 (Logs Retention 90 日統一のみ実装、メモリ/timeout/アラームは Cost Explorer 実測 $0 で動機消失のため保留) で PR を作成、chat/summarize は `defineFunction.logging.retention` で 1 行追加、evaluation は CDK の `logs.LogGroup` 新規 + `DockerImageFunction.logGroup` props 渡し。sandbox 検証で新 LogGroup 3 件 CREATE + Lambda 3 件 UPDATE が 124 秒で完走。/ **Issue #33** は PR #62 で完了 — KB 4 リソースに `RemovalPolicy.RETAIN` 明示 + `docs/operations.md` 新規 (Run Book 5 セクション、Issue #30 完了で 6 セクションに拡張) + `hosting.ts` コメント補強で #34 残作業も統合、sandbox Deployment 6.1 秒のメタデータ UPDATE のみ完走確認。/ 2026-05-23: Issue #31/#32 両方完了 (PR #60/#61)、予算 $30→$15 引き下げ、#34/#29 クローズ。これでコードレビュー由来の P1/P2 Issue (#28/#29/#30/#31/#32/#33/#34) は全消化。次は家族体感系 (Nova Pro クエリ拡張 `feature/query-expansion-issue-31`、Issue #21 👎+自由記述、Issue #16 Phase 3 LLM 補助分類) のどれか)
...
| ~~P2~~ | ~~#30~~ | ~~Lambda リソース・CloudWatch Logs 保持の実測ベース最適化~~ — **2026-05-24 スコープ縮小版で完了** (新規 PR `feature/issue-30-logs-retention`、knowledge.md 2026-05-24 参照)。Cost Explorer 実測 (2026-05-23、Lambda/CloudWatch 共に $0) で「メモリ/timeout 最適化はコスト削減動機ゼロ、Cold start 悪化リスクあり」「アラーム追加は家族のみ規模で監視疲れリスク」と判断し、**(3) Logs Retention 90 日統一のみ実装**、(2)/(4) は Issue コメントで保留宣言。3 Lambda の LogGroup を CDK で新規作成 (retention `THREE_MONTHS`) + 既存自動作成 LogGroup は孤児化 (手動 cleanup 手順は `docs/operations.md` §6) | コスト・信頼性・観測性 | `functions/*/resource.ts`, `infra/evaluation.ts`, `docs/operations.md` |
...
着手順の推奨: ~~#28~~ (2026-05-23 完了) → ~~#29~~ (2026-05-23 クローズ、前提解消済み) → ~~#34~~ (2026-05-23 クローズ、PR #49 で実装済み + 残作業は #33 統合) → ~~#31~~ (2026-05-23 完了、PR #60) → ~~#32~~ (2026-05-23 完了、PR #61) → ~~#33~~ (2026-05-24 完了、PR #62) → ~~#30~~ (2026-05-24 スコープ縮小版で完了、新規 PR `feature/issue-30-logs-retention`、メモリ/timeout/アラームは Cost Explorer 実測 $0 で動機消失のため保留)。コードレビュー由来の P1/P2 Issue は全消化済み。次は家族体感系 (Nova Pro クエリ拡張 / Issue #21 👎+自由記述 / Issue #16 Phase 3) のどれか。
